...
def events_npz(base_path):
    event_path = os.path.join(base_path, 'events')
    ...
    # 기본적인 이벤트 정보
    try:
        print(f"event file path : {event_files[0]}")
        data = np.load(event_files[0])
        events_stack = extract_events(data, 'bs_ergb')
        ...
        for key in data.keys():
            ...
            if key == 'timestamp':
                ...
                if arr.size > 0:
                    ...
                    x_data = convert_and_fix_event_pixels(arr, FRAME_WIDTH - 1)

                    print(f"======= aligned x")
                    print(f"- shape : {x_data.shape}")
                    print(f"- dtype : {x_data.dtype}")
                    print(f"- min : {float(x_data.min())}")
                    print(f"- max : {float(x_data.max())}")
                ...
                if arr.size > 0:
                    ...
                    y_data = convert_and_fix_event_pixels(arr, FRAME_HEIGHT - 1)

                    print(f"======= aligned y")
                    print(f"- shape : {y_data.shape}")
                    print(f"- dtype : {y_data.dtype}")
                    print(f"- min : {float(y_data.min())}")
                    print(f"- max : {float(y_data.max())}")
...
def image_event_align(base_path):
    ...
    # x, y 좌표 수정
    x_data = convert_and_fix_event_pixels(event['x'], FRAME_WIDTH - 1)
    y_data = convert_and_fix_event_pixels(event['y'], FRAME_HEIGHT - 1)
    p = event['polarity']
...
def main():
    base_path = '/scratch2/jiyun.kong/bs_ergb'
    print(f"기본 경로: {base_path}")

    tr_val_ts = ['train', 'val', '1_TEST']

    for tvt in tr_val_ts:
        base_tvt_path = os.path.join(base_path, tvt)
        seq_paths = glob.glob(os.path.join(base_tvt_path, '*/'))
